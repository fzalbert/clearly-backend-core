...
namespace SmsSender
{
    ...
    public class SmsRuProvider : ISmsProvider
    {
        ...
        String smtpLogin = "";                           // Логин для авторизации на SMTP-сервере.
        String smtpPassword = "";                     // Пароль для авторизации на SMTP-сервере.
        String smtpServer = "";                         // SMTP-сервер.
        Int32 smtpPort = Convert.ToInt32(0);             // Порт для авторизации на SMTP-сервере.
        Boolean smtpUseSSL = Convert.ToBoolean(false);     // Флаг - использовать SSL.
        Boolean translit = Convert.ToBoolean(false);         // Переводит все русские символы в латинские.
        Boolean test = Convert.ToBoolean(false);                 // Имитирует отправку сообщения для тестирования ваших программ на правильность обработки ответов сервера. При этом само сообщение не отправляется и баланс не расходуется.
        ...
        String log_sent = dir + "Sent.txt";
        String log_status = dir + "Status.txt";
        String log_cost = dir + "Cost.txt";
        String log_balance = dir + "Balance.txt";
        String log_limit = dir + "Limit.txt";
        String log_senders = dir + "Senders.txt";
        String log_auth = dir + "AuthCheck.txt";
        String log_error = dir + "Error.txt";
        String log_stoplist = dir + "Stoplist.txt";
        #endregion
        ...
        {
            return Send(from, new String[] { to }, text, DateTime.MinValue, EnumAuthenticationTypes.Simple);
        }
